...
```
Console.log is used to print information back to the console/terminal while return prints information back to where the call was made typically to the user.
```

**_What's the difference between defining and calling a function?_**

```
Defining a function doesn't run any of the code while calling a function will run the code inside the function.
```
...
```
Numbers can be positive or negative and are used to perform mathematical operations.
```

**_Boolean_**

```
Booleans are used to execute code based on conditional logic. There are only two values: true or false.
```
